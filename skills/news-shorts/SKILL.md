...
1. **ゴールデンタイム**: **7-9時**（通勤）、**12-13時**（昼休み）、**18-23時**（夜のニュース時間）。早朝5-7時は避ける
2. **1日最大3-4本**: 同じ時間帯に2本以上投稿しない。間隔を最低2時間あける。5本以上は自食い（cannibalization）発生。7本以上でアルゴリズムにスパム判定
3. **英語サブタイトル禁止**: `| Shadow Bank Collapse` 等は日本語ターゲットにノイズ。検索にも効かない

### [MUST] タイトル構築の黄金ルール — 「日本企業名 + 兆円 + 財布直撃」

タイトルの最初15文字で勝負が決まる。以下の3要素を含めること:

1. **日本の企業名・権威ある人物名** — 「損保ジャパン」「デンソー」「植田総裁」「トヨタ」。外国企業名だけ（`UBS`, `Blackstone`）は認知されない。外国ニュースでも日本との接点を示す（例: `三井住友に飛び火`）
2. **具体的な金額（兆円・万円・%）** — `1.3兆円`, `1ドル200円`, `年8万円赤字`。金額は最初の15文字以内に入れる
3. **財布直撃ワード** — `敵対買収`, `破綻`, `赤字`, `値上がり`, `ヤバい`。視聴者が「自分に関係ある」と思うフレーズ

#### タイトル判定チェック

| チェック項目 | ○ | × |
|------------|---|---|
| 最初の15文字に日本人が知る固有名詞？ | デンソーがローム敵対買収 | MFS二重担保で破綻 |
| 最初の15文字に金額？ | 1.3兆円規模の買収 | パワー半導体の再編主導 |
| 「自分の財布」を連想させる？ | ガソリンはどうなる | 石油備蓄制度の歴史 |

#### パフォーマンス実績（参考）

| パターン | 平均Views | 例 |
|---------|----------|-----|
| 日本企業名 + 兆円 + 財布直撃 | 1,000+ | `1ドル200円？`, `植田総裁が明言`, `デンソーがローム敵対買収` |
| 有名企業/人物 + 日本接点 | 700+ | `バークシャー日本商社`, `アンソロピック3兆円` |
| 海外ニュース + 日本角度弱い | 10-50 | `UBS警告`, `Blackstone払い戻し` |
| 外国企業名のみ（日本接点なし） | 5-30 | `MFS破綻`, `Blue Owl解約制限` |
| 早朝5-7時連投 | 5-20 | 3/4水曜の8本連投 → 全滅 |
| 1日5本以上 | 自食い発生 | 各動画の再生数が分散して全体低下 |
...
- **[MUST] text と texts の内容一致** — `text`（ナレーション）の内容を `texts`（字幕）で漏れなくカバーすること。ナレーションで言及する事実・数字・キーワードが字幕に含まれていないと視聴者が混乱する。texts のセグメント数は text の文数と必ずしも一致しなくてよいが、内容はすべてカバーする
...
Use 2-3 beats. **3ビート** が標準構成（HOOK + BODY + CLOSE）。シンプルなトピックは2ビートでOK。4ビート以上は50秒を超えやすいので注意。
...
This replaces the old relative path pattern (`../../../output/{topic}/images/script/{imageKey}.png`). The `image:` scheme works with any `-o` output directory.
...
13. **text と texts の内容一致**? (ナレーションの内容が字幕でカバーされている)
...
- `scripts/20260307/denso-rohm-tob/script.json` — 高パフォーマンス: 日本企業名+兆円+敵対買収
- `scripts/20260307/oil-reserve-release/script.json` — 財布直撃: 石油備蓄+ガソリン値上がり
- `scripts/20260307/toshiba-ai-gutted/script.json` — 日本企業+外資構造: 東芝の切り札売却
- `scripts/20260304/hormuz-japan-200yen/script.json` — 高パフォーマンス: 円安+具体数字
